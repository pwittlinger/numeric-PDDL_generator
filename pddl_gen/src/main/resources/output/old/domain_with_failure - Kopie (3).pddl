(define (domain trace-alignment)
  ...
  (:action check_added_variables_model
    :parameters (?a - activity ?s1 - automaton_state ?s2 - automaton_state)
    :precondition (and 
      (adding_value_model ?a ?s1)
      (cur_s_state ?s1)
      (complete_sync ?a)
  ...
  (:action move_in_model_move_automata
      ...
      (forall (?pn - parameter_name ?s1 - automaton_state ?s2 - automaton_state) 
        (imply (and 
            ...
            ; I don't know why this addition is crashing the heuristic, but it does.
            (and 
            ;(not (failure_state ?s2))
            (has_added_parameter_aut ?a ?pn ?s1)
            (or
      ...
      (forall (?s1 - automaton_state ?pn - parameter_name)
        (when (has_added_parameter_aut ?a ?pn ?s1)
        (and (not (has_added_parameter_aut ?a ?pn ?s1))
        ;(assign (added_parameter_aut ?a ?pn ?s1) 101)
         ))
      )

      (forall (?s1 - automaton_state ?pn - parameter_name)
        (when (adding_value_model ?a ?s1)
        (not (adding_value_model ?a ?s1))
        ;(assign (added_parameter_aut ?a ?pn ?s1) 101)
         )
      )
  ...
  (:action validate-payload
      ...
      :effect (and 
        ...
        ;Check if case parameter is missing
          ...
          ;  ; If parameter is missing
            (when (and 
            ;(not (petrinet_state ?s1))
            ;(not (petrinet_state ?s2))
            ;(not (petri_net_activity ?a))
            (cur_s_state ?s1)
            (automaton ?s1 ?a ?s2)
