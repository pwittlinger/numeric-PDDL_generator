(define (domain trace-alignment)
  ...
  ;; --------------------------------------------------------------------------------------------------
    ...
    :effect (and 
      ...
      (after_add)
      (complete_sync ?a)
      (increase (current_trace_length) 1))
  )
  ...
  (:action move_in_model_move_automata
      ...
      (when (> (current_trace_length) (defined_trace_length))
          (not (trace_length_goal)))
  ...
  (:action verify_trace_length
      :parameters ()
      :precondition (and
        (not (failure)))
      :effect (and 
        (when (= (defined_trace_length) (current_trace_length))
      (trace_length_goal))
      (when (> (defined_trace_length) (current_trace_length))
      (not (trace_length_goal)))
      )
