(define (domain trace-alignment)
  ...
  (:types automaton_state trace_state parameter_name value_name
        alphabet
        constraint_automaton activity - alphabet)
  ...
  (:predicates 
    ...
    (automaton ?s1 - automaton_state ?a - alphabet ?s2 - automaton_state)
    (cur_t_state ?t - trace_state)
    (cur_s_state ?s - automaton_state)
    (goal_state ?s - automaton_state)
    (inital_state ?s - automaton_state)
  ...
  (:action prefix-sync-finished
      :parameters ()
      :precondition (exists (?t1 - trace_state) 
                (and
                  (cur_t_state ?t1)
                  (final_t_state ?t1)))
  ...
  :precondition (and (cur_s_state ?s1) (failure_state ?s1) (inital_state ?s2) 
              (automaton ?s1 ?e ?s2)
              ;(failure)
              (violated ?e))
  :effect (and
